...
export function WalletConnect({children}) {
  ...
  return <div>
    {!account?
      <Button id="button-connect" onClick={() => {connect((p) => {setProvider(p)});}}>Connect Wallet</Button>
    :
    <Button id="button-connected" onClick={() => {connect((p) => {setProvider(p)});}}>{account}</Button>
    }
